...
//     const { domain, target } = req.query as { domain: string; target: string };
//     try {
...
// // 检查域名的 SSL 证书状态
// router.get('/check-ssl-status', async (req: Request, res: Response) => {
...
// // 生成并返回 DNS 信息
// router.get('/apply-ssl-certificate', async (req: Request, res: Response) => {
//     const { domain } = req.query as { domain: string };
...
// // 封装账户密钥的读取和保存逻辑
// async function ensureAccountKeyExists(accountKeyPath: string): Promise<string> {
//     if (!fs.existsSync(accountKeyPath)) {
...
// router.get('/register-account', async (req: Request, res: Response) => {
//     const accountKeyPath = path.join(__dirname, '../public/key/accountKey.pem');
